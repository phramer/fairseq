...
class BitextOutputFromGen(object):
    def __init__(
        ...
        if nbest:
            pred_source, pred_hypo, pred_score, pred_target, pred_pos_score = reprocess_nbest(
                predictions_bpe_file
            )
        else:
...
class LMOutput(object):
    def __init__(
        ...
        lm_sentences, lm_sen_scores, lm_sen_pos_scores, lm_no_bpe_sentences, lm_bpe_tokens = parse_lm(
            lm_score_file,
